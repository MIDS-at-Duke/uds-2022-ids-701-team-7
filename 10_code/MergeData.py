...
def merge_data(path):
    # load data files
    weather = pd.read_csv('weather.csv').drop(columns = 'Unnamed: 0')
    RPS = pd.read_csv('StatesRPS.csv').drop(columns = ['Percentage', 'TargetYear'])
    coal = pd.read_csv('20_intermediate_files/clean_coal.csv').drop(columns = 'Unnamed: 0')
    gas = pd.read_csv('20_intermediate_files/clean_gas.csv').drop(columns = 'Unnamed: 0')
    petroleum = pd.read_csv('00_source_data/petroleum price.csv')
    solar = pd.read_csv('00_source_data/solar-pv-prices.csv')
    generation = pd.read_csv('clean_energy_percent_processed.csv').drop(columns = 'Unnamed: 0')
    ...
    # change column names
    coal = coal.rename(columns = {'year': 'Yer', 'WPS051': 'CoalPrice'})
    solar = solar.drop(columns = ['Entity', 'Code']).rename(columns = {'Solar PV Module Cost (2019 US$ per W)': 'SolarPrice'})
    petroleum = petroleum.rename(columns = {'U.S. Crude Oil First Purchase Price (Dollars per Barrel)':'PetroleumPrice'})
